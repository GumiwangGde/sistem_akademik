<x-app-layout>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 sm:px-10 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('admin.dosen.store') }}" id="createDosenForm">
                        @csrf
                            ...
                            <div>
                                ...
                                <div class="mt-1 flex rounded-md shadow-sm">
                                    ...
                                    <span class="inline-flex items-center px-3 rounded-r-md border border-l-0 border-gray-300 bg-gray-50 text-gray-500 sm:text-sm">
                                        @it.lecturer.pens.ac.id {{-- Domain Dosen --}}
                                    </span>
                                </div>
                                @error('username')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <div class="md:col-span-2">
                                <div class="flex items-start">
                                    <div class="flex items-center h-5">
                                        <input id="is_dosen_wali" name="is_dosen_wali" type="checkbox" value="1" {{ old('is_dosen_wali') ? 'checked' : '' }}
                                               class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                                    </div>
                                    <div class="ml-3 text-sm">
                                        <label for="is_dosen_wali" class="font-medium text-gray-700">{{ __('Jadikan Dosen Wali?') }}</label>
                                        <p class="text-xs text-gray-500">Centang jika dosen ini dapat bertindak sebagai dosen wali.</p>
                                    </div>
                                </div>
                                @error('is_dosen_wali')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // const formEl = document.getElementById('createDosenForm'); // Tidak perlu submit listener jika controller membuat email
