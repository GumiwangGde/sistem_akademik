                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
                                <div>
                                    ...
                                    <select name="class_year" id="class_year" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>
                                        {{-- Tidak perlu opsi "-- Pilih --" karena ini edit dan field ini bagian dari nama kelas --}}
                                        @for ($i = 1; $i <= 4; $i++)
                                            <option value="{{ $i }}" {{ old('class_year', $kelas->class_year) == $i ? 'selected' : '' }}>{{ $i }}</option>
                                        @endfor
                                    </select>
                                </div>
                                ...
                                <div>
                                    ...
                                    <select name="program" id="program" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>
                                        <option value="D3" {{ old('program', $kelas->program) == 'D3' ? 'selected' : '' }}>D3</option>
                                        <option value="D4" {{ old('program', $kelas->program) == 'D4' ? 'selected' : '' }}>D4</option>
                                        <option value="D3 PJJ" {{ old('program', $kelas->program) == 'D3 PJJ' ? 'selected' : '' }}>D3 PJJ</option>
                                        <option value="D4 PJJ" {{ old('program', $kelas->program) == 'D4 PJJ' ? 'selected' : '' }}>D4 PJJ</option>
                                        <option value="S2" {{ old('program', $kelas->program) == 'S2' ? 'selected' : '' }}>S2</option>
                                        <option value="S3" {{ old('program', $kelas->program) == 'S3' ? 'selected' : '' }}>S3</option>
                                    </select>
                                </div>
                                ...
                                <div>
                                    ...
                                    <select name="prodi" id="prodi" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>
                                        <option value="IT" {{ old('prodi', $kelas->prodi) == 'IT' ? 'selected' : '' }}>IT</option>
                                        <option value="ELIN" {{ old('prodi', $kelas->prodi) == 'ELIN' ? 'selected' : '' }}>ELIN</option>
                                        <option value="ELKA" {{ old('prodi', $kelas->prodi) == 'ELKA' ? 'selected' : '' }}>ELKA</option>
                                ...
                                <div>
                                    ...
                                    <select name="class_type" id="class_type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>
                                        @foreach (range('A', 'J') as $type)
                                            <option value="{{ $type }}" {{ old('class_type', $kelas->class_type) == $type ? 'selected' : '' }}>{{ $type }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div>
                                    ...
                                    <select name="status" id="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>
                                        <option value="active" {{ old('status', $kelas->status) == 'active' ? 'selected' : '' }}>Active</option>
                                        <option value="inactive" {{ old('status', $kelas->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                                    </select>
                                </div>

                                <div id="dosen_wali_div"> {{-- Visibilitas awal diatur oleh JS --}}
                                    ...
                                    <select name="id_dosen_wali" id="id_dosen_wali" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                                        ...
                                        @foreach($dosen as $dosenItem)
                                            <option value="{{ $dosenItem->id_dosen }}" {{ old('id_dosen_wali', $kelas->id_dosen_wali) == $dosenItem->id_dosen ? 'selected' : '' }}>{{ $dosenItem->user->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>

                            <input type="hidden" name="nama_kelas" id="nama_kelas" value="{{ old('nama_kelas', $kelas->nama_kelas) }}">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function updateClassName() {
                ...
                // Hanya update jika semua komponen nama kelas sudah dipilih (penting untuk form create)
                // Untuk edit, kita bisa langsung bentuk namanya karena field sudah terisi.
                if (classYear && program && prodi && classType) {
                    namaKelasInput.value = `${classYear} ${program} ${prodi} ${classType}`;
                } else {
                     // Untuk edit, kita mungkin ingin tetap menampilkan nama_kelas lama jika salah satu field dikosongkan
                     // Namun, karena field ini required, idealnya tidak akan pernah kosong.
                     // Jika ingin fallback ke nama_kelas awal jika ada yang dikosongi:
                     // namaKelasInput.value = "{{ $kelas->nama_kelas }}"; // Atau biarkan kosong
                     namaKelasInput.value = ''; // Atau kosongkan jika tidak lengkap
                }
            }

            function toggleDosenWali() {
                const isDosenWaliRequired = statusSelect.value === 'active';
                dosenWaliDiv.style.display = isDosenWaliRequired ? 'block' : 'none';
                dosenWaliSelect.required = isDosenWaliRequired;
                dosenWaliRequiredStar.style.display = isDosenWaliRequired ? 'inline' : 'none';
                
                if (!isDosenWaliRequired) {
                    // Untuk form edit, JANGAN reset value dosen wali jika status inactive
                    // kecuali jika memang diinginkan. Biasanya, kita ingin menyimpan pilihan sebelumnya.
                    // Jika ingin direset: dosenWaliSelect.value = ''; 
                }
            }
            ...
            updateClassName(); // Untuk memastikan nama_kelas terisi dari value awal dropdown
            toggleDosenWali(); // Untuk mengatur visibilitas & requirement dosen_wali
        });
    </script>
    @endpush
</x-app-layout>