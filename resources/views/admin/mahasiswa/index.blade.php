                ...
                <div class="px-4 py-4 sm:px-6 sm:py-5"> {{-- Padding internal header card disesuaikan --}}
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <h1 class="text-xl font-bold text-gray-900">{{ __('Manajemen Mahasiswa') }}</h1>
                                <p class="text-sm text-gray-500">Kelola data mahasiswa dalam sistem akademik</p>
                            </div>
                        </div>
                        <a href="{{ route('mahasiswa.create') }}" class="inline-flex items-center justify-center px-4 py-2 bg-blue-600 border border-transparent rounded-lg font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 active:bg-blue-800 focus:outline-none focus:border-blue-800 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition ease-in-out duration-150">
            ...
            <div class="space-y-6"> {{-- Memberi jarak antar elemen flash dan card utama --}}
                ...
                @if (session('success'))
                    <div class="flex w-full overflow-hidden bg-green-50 rounded-lg shadow-sm border border-green-300">
                        ...
                        <div class="px-4 py-3"> {{-- Padding teks flash disederhanakan --}}
                ...
                @endif
                {{-- Jika Anda juga menggunakan session('error'), tambahkan blok serupa di sini --}}


                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200">
                    <div class="p-4 sm:p-6"> {{-- Padding internal card utama disesuaikan --}}
                        @if(isset($mahasiswa) && $mahasiswa->isNotEmpty())
                            <div class="flex flex-col">
                                <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                                    <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                                        <div class="shadow-sm border border-gray-200 sm:rounded-lg overflow-hidden"> {{-- Styling wrapper tabel disesuaikan --}}
                                                    ...
                                                    @foreach($mahasiswa as $item)
                                                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                                                            ...
                                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                                                {{ $item->user->name ?? ($item->nama ?? 'N/A') }} {{-- Menggunakan user->name jika ada, fallback ke item->nama --}}
                                                            </td>
                                                            ...
                                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                                                <div class="flex items-center justify-center space-x-2">
                                                                    <a href="{{ route('mahasiswa.edit', $item->id_mahasiswa) }}" class="text-blue-600 hover:text-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 p-1 rounded-md" title="Edit">
                                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                                        </svg>
                                                                    </a>
                                                                    <form action="{{ route('mahasiswa.destroy', $item->id_mahasiswa) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus mahasiswa {{ $item->user->name ?? ($item->nama ?? '') }}?')">
                                                                        @csrf
                                                                        @method('DELETE')
                        ...
                        @else
                            <div class="bg-white p-6 sm:p-8 rounded-lg shadow-sm border border-gray-200 text-center">
                                <div class="inline-flex items-center justify-center w-16 h-16 bg-blue-100 rounded-full mb-4"> {{-- Warna ikon disesuaikan --}}
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                </div>
                                <h2 class="text-xl font-semibold text-gray-800 mb-2">Belum Ada Mahasiswa</h2>
                                <p class="text-gray-600 mb-6">Silakan tambahkan data mahasiswa baru dengan mengklik tombol "Tambah Mahasiswa"</p> {{-- Warna teks disesuaikan --}}
