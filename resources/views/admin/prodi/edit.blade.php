    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Edit Program Studi: ') }} <span class="text-indigo-600">{{ $prodi->nama_prodi }}</span>
            </h2>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 sm:px-10 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('admin.prodi.update', $prodi->id_prodi) }}">
                        @csrf
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <select name="jenjang" id="jenjang" 
                                    ...
                                    @php
                                        // $jenjangList dikirim dari ProdiController@edit
                                        $jenjangOptions = $jenjangList ?? ['D1', 'D2', 'D3', 'D4', 'S1', 'S2', 'S3', 'Profesi', 'Spesialis'];
                                    @endphp
                                    @foreach ($jenjangOptions as $jenjang_option)
                                        <option value="{{ $jenjang_option }}" {{ old('jenjang', $prodi->jenjang) == $jenjang_option ? 'selected' : '' }}>
                                            {{ $jenjang_option }}
                                        </option>
                                    @endforeach
                                </select>
