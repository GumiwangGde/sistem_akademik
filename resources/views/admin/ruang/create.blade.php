{{-- resources/views/admin/ruang/create.blade.php --}}
<x-app-layout>
    <x-slot name="header">
                ...
                <div class="px-4 py-4 sm:px-6 sm:py-5"> {{-- Padding internal header card disesuaikan --}}
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <h1 class="text-xl font-bold text-gray-900">{{ __('Tambah Ruang') }}</h1>
                                <p class="text-sm text-gray-500">Tambahkan ruangan baru ke sistem</p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200">
                <div class="p-6 sm:p-8">
                    {{-- Flash Message Success (jika ada setelah redirect dari tempat lain) --}}
                    @if(session('success'))
                        <div class="mb-6 flex w-full overflow-hidden bg-green-50 rounded-lg shadow-sm border border-green-300">
                    ...
                    <form action="{{ route('admin.ruang.store') }}" method="POST" class="space-y-6">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
                            <div>
                                ...
                                <input type="text" name="nama_ruang" id="nama_ruang" value="{{ old('nama_ruang') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" placeholder="Contoh: Lab RPL Gedung D401" required>
                                @error('nama_ruang')
                            ...
                            <div>
                                <label for="kapasitas" class="block text-sm font-medium text-gray-700 mb-1">Kapasitas <span class="text-red-500">*</span></label>
                                <input type="number" name="kapasitas" id="kapasitas" value="{{ old('kapasitas') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" placeholder="Masukkan kapasitas ruang" required min="1">
                                @error('kapasitas')
                                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="flex items-center justify-end pt-4 space-x-3"> {{-- justify-end dan pt-4 untuk spasi --}}
                            <a href="{{ route('admin.ruang.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-200 border border-gray-300 rounded-lg font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-300 active:bg-gray-400 focus:outline-none focus:border-gray-400 focus:ring-2 focus:ring-gray-300 focus:ring-offset-1 transition ease-in-out duration-150">
