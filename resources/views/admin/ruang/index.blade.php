                    ...
                    <div class="p-4 sm:p-6"> {{-- Padding internal card utama disesuaikan --}}
                            ...
                            <div class="flex flex-col">
                                <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                                    <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                                                    ...
                                                    @foreach($ruang as $item)
                                                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                                {{-- Penomoran dengan paginasi (jika $ruang adalah Paginator) --}}
                                                                @if(method_exists($ruang, 'currentPage'))
                                                                    {{ $loop->iteration + ($ruang->currentPage() - 1) * $ruang->perPage() }}
                                                                @else
                                                                    {{ $loop->iteration }}
                                                                @endif
                                                            </td>
                                                            ...
                                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                                                <div class="flex items-center justify-center space-x-2">
                                                                    <a href="{{ route('admin.ruang.edit', $item->id) }}" class="text-blue-600 hover:text-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 p-1 rounded-md" title="Edit">
                                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                                        </svg>
                                                                    </a>
                                                                    <form action="{{ route('admin.ruang.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus ruang {{ $item->nama_ruang }}?')">
                                                                        @csrf
                                                                        @method('DELETE')
                                        ...
                                        {{-- Paginasi --}}
                                        @if (method_exists($ruang, 'hasPages') && $ruang->hasPages())
                                            <div class="mt-6">
                                                {{ $ruang->links() }}
                                            </div>
                                        @endif
                        ...
                        @else
                            <div class="bg-white p-6 sm:p-8 rounded-lg shadow-sm border border-gray-200 text-center">
                                ...
                                <p class="text-gray-600 mb-6">Silakan tambahkan ruang baru dengan mengklik tombol "Tambah Ruang"</p> {{-- Warna teks disesuaikan --}}
